...
async function generateFastAffiliateLinks() {
  console.log("=================================================");
  console.log("   ⚡ [쿠팡 파트너스] 간편 링크 만들기 자동 생성");
  console.log("=================================================\n");
  ...
  console.log("🔗 간편 링크 만들기 이동 중...");
  await page.goto("https://partners.coupang.com/#affiliate/ws/link-generator", { waitUntil: "networkidle2" });
  await sleep(3000);

  const results = {};

  for (const item of targets) {
    console.log(`\n🎯 [${item.id}] ${item.name} 링크 변환 시도...`);
    
    // 입력창 탐색
    const inputHandle = await page.$("input[type='text'], textarea, input.url-input");
    if (inputHandle) {
      await page.evaluate((val) => {
        const input = document.querySelector("input[type='text'], textarea, input.url-input");
        if (input) {
          input.value = "";
          input.focus();
        }
      });
      await inputHandle.type(item.coupangUrl, { delay: 10 });
      await sleep(500);
      ...
      console.log(`  -> 링크 생성 버튼 클릭: ${clicked}`);
      await sleep(2000);

      // 생성된 파트너스 단축 URL 탐색
      const shortenLink = await page.evaluate(() => {
        const allTextElements = Array.from(document.querySelectorAll("input, textarea, p, span, div, a"));
      ...
      if (shortenLink) {
        console.log(`  ✅ [수익 링크 획득] ${shortenLink}`);
        results[item.id] = shortenLink;
      } else {
        console.log(`  ⚠️ 링크 탐색 실패, 기본 파트너스 링크 유지`);
        results[item.id] = "https://link.coupang.com/a/dRiMJOFU0i";
      }
    } else {
      console.log(`  ❌ 입력창을 찾지 못함`);
      results[item.id] = "https://link.coupang.com/a/dRiMJOFU0i";
    }
    await sleep(1000);
  }

  const outPath = resolve(process.cwd(), "public/exact_coupang_partners_links.json");
  writeFileSync(outPath, JSON.stringify(results, null, 2), "utf-8");
  console.log(`\n💾 최종 수익 링크 매핑 완료: ${outPath}`);

  await page.close();
}

generateFastAffiliateLinks().catch(console.error);
