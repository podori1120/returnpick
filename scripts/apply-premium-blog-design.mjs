...
import { renderPremiumPostHtml, renderPremiumDealCard } from "@/lib/premiumBlogDesign";
...
async function applyPremiumDesignToAllPosts() {
  console.log("=================================================");
  console.log("   블로그 포스팅 프리미엄 세련된 디자인 전면 적용");
  console.log("=================================================\n");
  ...
  // 1. 메인 종합 포스트 (35종/전체 핫딜) 프리미엄 매거진 디자인 갱신
  const mainPostId = "5479330542488219707";
  console.log(`[1/5] 메인 종합 포스트(${mainPostId}) 프리미엄 디자인 갱신 중...`);

  const premiumMainHtml = renderPremiumPostHtml(
    "오늘의 쿠팡 핫딜 & 폴센트 60일 최저가 검증 모음",
    "실시간 가격 추적 엔진으로 60일간의 가격 변동과 역대 최저가를 검증한 엄선 특가 큐레이션입니다.",
  ...
  for (const [idx, p] of singlePosts.entries()) {
    console.log(`[${idx + 2}/5] 단독 포스트(${p.id}) 프리미엄 디자인 갱신 중...`);
  ...
  // 3. 검색엔진 즉시 색인 재전송
  console.log("\n[검색엔진] IndexNow API에 디자인 개편 URL 전송 중...");
  try {
    ...
    console.log("  -> ✅ IndexNow 색인 요청 완료!");
  } catch (e) {}

  console.log("\n=================================================");
  console.log("🎉 블로그 5개 전체 포스팅 프리미엄 디자인 전면 적용 완료!");
  console.log("실시간 확인: https://returnpick-deals.blogspot.com/2026/08/18.html");
  console.log("=================================================");
}

applyPremiumDesignToAllPosts().catch(console.error);
