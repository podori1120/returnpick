...
여러 상품을 한 번에 운영할 때는 `/admin`의 `상품별 파트너스 링크 보강`에서 링크를 붙여넣고 `보이는 링크 n건 확인`을 먼저 실행합니다. 상품번호가 `MATCH` 또는 `MANUAL_CONFIRMED`로 확인된 항목만 `확인된 링크 n건 게시` 버튼에 포함되며, 게시 요청에서도 공개 품질 게이트를 다시 통과해야 합니다. 불일치 링크·승인용 샘플 링크·가격이나 이미지가 부족한 상품은 자동 게시되지 않습니다.
