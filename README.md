...
상품 상세의 `내가 정한 가격 저장`은 로그인 없이 현재 브라우저에 목표가와 저장 당시 확인가를 보관하고, `/watchlist` 가격 기준함에서 공개 상품의 최신 확인가와 다시 비교합니다. 가격 기준함의 `다시 확인` 버튼으로 현재 공개 상품 데이터를 수동 재조회할 수 있으며, 저장 당시 대비 상승·하락 폭도 실제 값이 있을 때만 표시합니다. 문자·푸시·이메일 알림을 보내는 기능은 아니며, 가격 기준함에는 공개 목록에서 확인되지 않는 상품을 별도로 표시합니다. 가격·재고·반품등급은 구매 직전 쿠팡 상품 페이지에서 확인하세요. 상세와 딜 카드, `/watchlist`와 `/compare`의 가격 시점 신호는 최신 확인 상태가 신선하고 동일 가격 기준 ReturnPick 관찰이 최근 30일 안에 2회 이상 있을 때만 `최근 관찰 최저`, `좋은 구매 시점`, `평균보다 낮음`으로 표시하며, 시장 전체 최저가로 해석하지 않습니다.
