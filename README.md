...
상품 상세의 `내가 정한 가격 저장`은 로그인 없이 현재 브라우저에 목표가와 저장 당시 확인가를 보관하고, `/watchlist` 가격 기준함에서 공개 상품의 최신 확인가와 다시 비교합니다. 가격 기준함의 `다시 확인` 버튼으로 현재 공개 상품 데이터를 수동 재조회할 수 있으며, 저장 당시 대비 상승·하락 폭도 실제 값이 있을 때만 표시합니다. 목표가 도달 상품은 화면에 즉시 모아 보여주고, 사용자가 직접 허용한 브라우저에서만 저장 가능한 범위 내 같은 확인값의 중복을 줄여 알림을 표시합니다. 백그라운드 가격 추적이나 문자·이메일 발송은 하지 않으며, 가격 기준함에는 공개 목록에서 확인되지 않는 상품을 별도로 표시합니다. 가격·재고·반품등급은 구매 직전 쿠팡 상품 페이지에서 확인하세요. 상세와 딜 카드, `/watchlist`와 `/compare`의 가격 시점 신호는 최신 확인 상태가 신선하고 동일 가격 기준 ReturnPick 관찰이 최근 30일 안에 2회 이상 있을 때만 `최근 관찰 최저`, `좋은 구매 시점`, `평균보다 낮음`으로 표시하며, 시장 전체 최저가로 해석하지 않습니다.
...
상품 URL을 함께 입력한 경우에는 `공개 웹 정보 보강 시도`를 선택할 수 있습니다. 이 기능은 기본적으로 꺼져 있으며, 체크했을 때만 입력한 **그 상품 URL 하나**를 `PUBLIC_WEB_ALLOWED_HOSTS`와 `robots.txt`가 허용하는 범위에서 한 번 확인합니다. 페이지에서 실제로 읽힌 제목, 이미지, 명시적으로 라벨된 일반 가격, 반품가·등급·재고 표현만 참고 후보로 채우고, `new_price`나 제휴 링크는 추정하거나 만들지 않습니다. 보강에는 짧은 전체 시간 예산과 본문 읽기 제한이 있으며, 결과는 항상 `needs_review`·비공개 후보로 저장됩니다. URL을 입력하지 않았거나 설정이 꺼져 있거나 접근이 제한되면 기존 입력값만 저장합니다. 원문 HTML은 저장하지 않고, 상태·채워진 필드·간단한 진단과 구조화된 `web_return_info`/`public_web_inspection` provenance만 `raw_json`에 남깁니다. 일괄 등록에서도 같은 체크박스가 적용되며, 공개 웹 보강을 선택한 행은 서버 요청당 최대 2개씩 처리합니다.
...
npm run public-web:intake:check
```
