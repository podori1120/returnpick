...
상품 상세의 `내가 정한 가격 저장`은 로그인 없이 현재 브라우저에 목표가를 보관하고, `/watchlist` 가격 기준함에서 공개 상품의 최신 확인가와 다시 비교합니다. 문자·푸시·이메일 알림을 보내는 기능은 아니며, 가격 기준함에는 공개 목록에서 확인되지 않는 상품을 별도로 표시합니다. 가격·재고·반품등급은 구매 직전 쿠팡 상품 페이지에서 확인하세요. 상세의 가격 기록에는 ReturnPick이 실제로 관찰한 최근 30일 가격만 `최근 30일 최저 관찰가`로 표시하며, 시장 전체 최저가로 해석하지 않습니다.
