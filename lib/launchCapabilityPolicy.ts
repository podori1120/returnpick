...
export const CATALOG_LAUNCH_ITEM_IDS = ["site", "approval_link", "admin_password", "bootstrap_catalog"] as const;
...
export function evaluateCatalogLaunchReadiness(items: CapabilityReadinessItem[]) {
  const itemById = new Map(items.map((item) => [item.id, item]));
  return CATALOG_LAUNCH_ITEM_IDS.every((id) => itemById.get(id)?.state === "ready");
}
