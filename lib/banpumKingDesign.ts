/**
 * 👑 [반품왕 에디션] 100~300만원대 초고단가 쿠팡 반품-미개봉/최상급 전용 렌더러
 * - 반품-미개봉(새것과 동일) & 반품-최상(단순변심 1회) 등급 보증
 * - 새상품 정가 대비 최소 40만원 ~ 최대 160만원 파격 절약액 하이라이트
 * - 단 1건 결제로 15만원 파트너스 목표 실적 즉시 달성
 */

export function renderBanpumKingCard(deal: {
  id: string;
  title: string;
  category: string;
  ...
  const isUnopened = deal.return_grade.includes("미개봉");

  return `
  <div style="margin-bottom: 36px; background: #ffffff; border-radius: 20px; border: 2px solid #2563eb; box-shadow: 0 8px 30px rgba(37,99,235,0.12); overflow: hidden; font-family: -apple-system, BlinkMacSystemFont, 'Pretendard', sans-serif;">
    <!-- 카드 헤더 (반품왕 공식 등급 배지) -->
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 14px 22px; background: linear-gradient(90deg, #0f172a 0%, #1e3a8a 100%); color: #ffffff;">
      ...
      <span style="background: #dc2626; color: #ffffff; font-size: 11px; font-weight: 800; padding: 4px 12px; border-radius: 20px;">
    ...
    <div style="padding: 26px 22px;">
      ...
      <div style="display: flex; flex-direction: row; gap: 20px; flex-wrap: wrap; margin-bottom: 18px;">
        ...
        <div style="flex: 1 1 250px; display: flex; flex-direction: column; justify-content: center;">
          <span style="color: #2563eb; font-size: 12px; font-weight: 800; margin-bottom: 4px;">${deal.category}</span>
      ...
      <div style="background: #f8fafc; border: 1.5px solid #cbd5e1; border-radius: 14px; padding: 18px; margin-bottom: 16px;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 13px; color: #64748b; border-top: 1px dashed #cbd5e1; padding-top: 10px;">
          <span>새제품 정상가: ${deal.new_product_price.toLocaleString()}원</span>
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 14px; background: #fffbeb; border: 1px solid #fef3c7; border-radius: 10px; font-size: 12px; color: #92400e; margin-bottom: 18px;">
        ...
        <span style="color: #dc2626; font-weight: 800;">⚡ 실시간 잔여 ${deal.stock_remain}대 한정</span>
      </div>

      <!-- CTA 버튼 -->
      <a href="https://returnpick.vercel.app/deals/${deal.id}?utm_source=blogger&utm_medium=owned&utm_campaign=banpum_king" target="_blank" rel="nofollow noopener" style="display: block; text-align: center; background: linear-gradient(135deg, #1e3a8a 0%, #2563eb 100%); color: #ffffff; text-decoration: none; font-size: 16px; font-weight: 900; padding: 16px 22px; border-radius: 12px; box-shadow: 0 6px 18px rgba(37,99,235,0.35); letter-spacing: -0.3px;">
        💎 쿠팡 반품 재고 &amp; 상태 확인하기 (30일 무료반품)
      </a>
    </div>
  </div>`;
}

export function renderBanpumKingMagazineHtml(title: string, subtitle: string, deals: any[]): string {
  const cardsHtml = deals.map((d, i) => renderBanpumKingCard(d, i)).join("\n");
  ...
  <article style="max-width: 740px; margin: 0 auto; font-family: -apple-system, BlinkMacSystemFont, 'Pretendard', sans-serif; color: #1e293b; line-height: 1.6;">
    ...
    <!-- 반품왕 히어로 배너 -->
    <div style="background: linear-gradient(135deg, #090d16 0%, #0f172a 50%, #1e3a8a 100%); border-radius: 20px; padding: 30px 24px; text-align: center; color: #ffffff; margin-bottom: 32px; box-shadow: 0 10px 30px rgba(15,23,42,0.35);">
      ...
      <div style="display: inline-flex; align-items: center; gap: 8px; background: rgba(0,0,0,0.3); padding: 8px 18px; border-radius: 10px; font-size: 13px; font-weight: 800; color: #38bdf8;">
        <span>🛡️ 쿠팡 공식 <strong>30일 무료 교환/반품 안심 보증</strong></span>
      </div>
    </div>
    ...
    <footer style="margin-top: 40px; padding: 22px; background: #f8fafc; border-radius: 14px; border: 1px solid #e2e8f0; font-size: 12px; color: #64748b; line-height: 1.65;">
      <p style="margin: 0 0 8px 0; font-weight: 800; color: #334155; font-size: 13px;">📌 반품왕 구매 가이드</p>
      <ul style="margin: 0; padding-left: 18px;">
        <li>'반품-미개봉'은 단순 박스 라벨만 개봉되거나 비닐 미개봉 상태로 본체는 100% 신품입니다.</li>
        <li>'반품-최상'은 단순 변심으로 단 1회 개봉된 특A급으로 새상품과 동일한 무상 AS가 적용됩니다.</li>
        <li>쿠팡 와우회원은 수령 후 30일간 무료 반품이 가능하여 제품 확인 후 부담 없이 결정하실 수 있습니다.</li>
      </ul>
    </footer>
  </article>`;
}
