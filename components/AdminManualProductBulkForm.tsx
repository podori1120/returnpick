...
import { ArrowRight, Copy, FileCheck2, ShieldCheck, Upload } from "lucide-react";
...
export default function AdminManualProductBulkForm({ password, onCreated }: { password: string; onCreated: () => void }) {
  ...
  const [recentAffiliateLinkCount, setRecentAffiliateLinkCount] = useState(0);
  ...
  async function importCandidates() {
    ...
    setResult(null);
    setRecentProductIds([]);
    setRecentAffiliateLinkCount(0);
    setNotice({ type: "info", message: "상품 URL·파트너스 링크 형식과 중복을 확인하고 있습니다." });
    try {
      ...
      setRecentAffiliateLinkCount(
        entries
          .split(/\r?\n/g)
          .map((entry) => entry.split("\t")[3]?.trim() ?? "")
          .filter((value) => isUsableAffiliateUrl(value)).length
      );
          ...
          {recentProductIds.length ? (
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-md border border-pine/30 bg-white px-3 py-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {recentAffiliateLinkCount > 0 ? (
                  <button
                    className="focus-ring inline-flex items-center gap-2 rounded-lg border border-pine/30 bg-pine/5 px-3 py-2 text-xs font-black text-pine hover:bg-pine/10"
                    onClick={openAdminAffiliateLinkQueue}
                    type="button"
                  >
                    <ShieldCheck size={14} aria-hidden /> 링크 목적지 확인 큐
                  </button>
                ) : null}
              </div>
            </div>
